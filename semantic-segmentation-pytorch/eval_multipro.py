...
def visualize_result(data, pred, dir_result):
    ...
    semantic_dir_result = dir_result + "_semantic"
    semantic_name = img_name.replace('rgb', 'semantic')
    Image.fromarray(im_vis).save(os.path.join(dir_result, img_name))
    Image.fromarray(pred_color).save(os.path.join(semantic_dir_result, semantic_name))
...
def main(cfg, gpus):
    ...
    print('Mean IoU: {:.4f}, Accuracy: {:.2f}%'
          .format(iou.sum()/num, acc_meter.average()*100))
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        "--cfg",
        default="config/ade20k-resnet101-upernet.yaml",
        metavar="FILE",
    ...
    if not os.path.isdir(os.path.join(dataset_dir, "result")):
        os.makedirs(os.path.join(dataset_dir, "result"))

    if not os.path.isdir(os.path.join(dataset_dir, "result_semantic")):
        os.makedirs(os.path.join(dataset_dir, "result_semantic"))
