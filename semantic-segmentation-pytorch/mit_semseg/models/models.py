...
class ModelBuilder:
    ...
    def build_decoder(arch='ppm_deepsup',
        ...
        if len(weights) > 0:
            ...
            pretrain_dict = torch.load(weights, map_location=lambda storage, loc: storage)
            net_decoder_dict = net_decoder.state_dict()
            for key, value in pretrain_dict.items():
                if net_decoder_dict[key].shape == pretrain_dict[key].shape:
                    net_decoder_dict[key] = pretrain_dict[key]

            net_decoder.load_state_dict(net_decoder_dict, strict=False)
